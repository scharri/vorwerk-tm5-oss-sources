...
void usage(void)
{
	...
	"\n"
	"  get_sw_version [-0|1] ................... TM41: get SW version of selected image\n"
	"    -0|1 .................................. \n"
	"\n"
	"  get_update_status........................ TM41: get update status saved in flash\n"
	"\n"
	"  set_update_status [-v] [-0|1|2|3|4|5|6|7] TM41: change update status on flash\n"
	"    -v .................................... Verbose mode\n"
	"    -0|1|2|3|4|5|6|7....................... \n"
	"\n"
	...
	"    -w .................................... Commit to flash\n"
	"    --date=xxxxxxxxxxxx ................... TM41: SW version date eg. 201212171845 (12 digits)\n"
	"    --comment=xxxxxxxxxxxx ................ TM41: SW version comment eg. 'build 1666' (max. 29 characters)\n"
	"\n"
...
int get_update_status_main(int argc, char **argv)
{
	int i, r;
	struct mtd_data *md;
	int flags = 0;
	...
	// write to stdout, so other script can receive it
	printf ("status=%d\n", md->fcb.TM41_Block.m_u32UpdateStatus);
...
void mtd_parse_sw_args(struct mtd_config *cfg, int argc, char **argv, struct sw_version *sw_version_params)
{
	int i;
	char *p, *s;

	memset (sw_version_params->version_date, '\0', 20);
	memset (sw_version_params->version_comment, '\0', 30);
	...
	for (i = 1; i < argc; i++)
	{
		...
		if (strlen("date") == (s - p) && memcmp("date", p, s - p) == 0)
		{
		    memcpy (sw_version_params->version_date, s+1, (strlen(s+1) > 20) ? 20 : strlen(s+1));
		}
		if (strlen("comment") == (s - p) && memcmp("comment", p, s - p) == 0)
		{
		    memcpy (sw_version_params->version_comment, s+1, (strlen(s+1) > 30) ? 30 : strlen(s+1));
		}
	    }
	}
}
...
int get_sw_version_main(int argc, char **argv)
{
	int image_nr = -1;
	int i, r;
	struct mtd_data *md;
	...
	// write to stdout, so other script can receive it
	printf ("date=%s;comment=%s\n", md->fcb.TM41_Block.images[image_nr].version_date, md->fcb.TM41_Block.images[image_nr].version_comment);
...
int extract_main(int argc, char **argv)
{
	...
	chip = 0;
	startpage = image == 0 ?
		md->curr_fcb->FCB_Block.m_u32Firmware1_startingSector :
		md->curr_fcb->FCB_Block.m_u32Firmware2_startingSector;
	size =  image == 0 ?
		md->curr_fcb->FCB_Block.m_u32SectorsInFirmware1 :
		md->curr_fcb->FCB_Block.m_u32SectorsInFirmware2;
...
// MICHAL - disable exit when bootstream verify fails
////	if (r != 0) {
...
static int perform_bootstream_update(struct mtd_data *md, FILE *infp, int image_mask, struct sw_version *sw_version_params)
{
	int i, r;
	...
	for (i = 0; i < 2; i++) {
		...
		if (i == 0) {
			start = md->fcb.FCB_Block.m_u32Firmware1_startingSector  * mtd_writesize(md);
			end = md->fcb.FCB_Block.m_u32Firmware2_startingSector * mtd_writesize(md);
		} else {
			start = md->fcb.FCB_Block.m_u32Firmware2_startingSector * mtd_writesize(md);
			end = mtd_size(md);
		}
		avail = end - start;
		...
		/* now update size */
		if (i == 0)
			md->fcb.FCB_Block.m_u32SectorsInFirmware1 = (size + 2047) / 2048;
		else
			md->fcb.FCB_Block.m_u32SectorsInFirmware2 = (size + 2047) / 2048;
		update |= UPDATE_BS(i);
		
		// update version date and comment
		memcpy (&md->fcb.TM41_Block.images[i].version_date, sw_version_params->version_date, 20);
		memcpy (&md->fcb.TM41_Block.images[i].version_comment, sw_version_params->version_comment, 30);
	}

	r = v1_rom_mtd_commit_structures(md, infp, update);
	if (r < 0) {
...
int update_main(int argc, char **argv)
{
	...
	int device_key;
	uint8_t *keyp;
	struct sw_version sw_version_params = {{0}};
	...
	mtd_parse_sw_args(&cfg, argc, argv, &sw_version_params);
...
////	}
	...
	r = perform_bootstream_update(md, infp, image_mask, &sw_version_params);
	if (r != 0) {
...
static int perform_update_status_commit(struct mtd_data *md, int update_status)
{
	int r;
	...
	if (md->curr_fcb->TM41_Block.m_u32UpdateStatus == update_status) {
		printf ("Update status %d already committed\n", update_status);
		return 0;
	}

	// change committed image
	md->curr_fcb->TM41_Block.m_u32UpdateStatus = update_status;

	r = v1_rom_mtd_commit_structures(md, NULL, UPDATE_FCB_ONLY);
	if (r < 0) {
		fprintf(stderr, "FAILED to commit structures\n");
		return -1;
	}
...
int set_update_status_main(int argc, char **argv)
{
	int i, r;
	struct mtd_data *md;
	int flags;
	...
	printf ("Will change update status to %d\n", update_status);
	...
	r = perform_update_status_commit (md, update_status);
	if (r != 0) {
		fprintf(stderr, "Unable to perform update status commit\n");
		usage();
	}
...
////	}
	...
	switch (rom_version) {
		case ROM_Version_0:
			...
			break;
		case ROM_Version_1:
			r = v1_rom_mtd_init(md, infp, &sw_version_params);
			break;
		case ROM_Version_2:
...
int main(int argc, char **argv)
{
	...
	if (strcmp(argv[0], "get_sw_version") == 0)
		return get_sw_version_main(argc, argv);

	if (strcmp(argv[0], "get_update_status") == 0)
		return get_update_status_main(argc, argv);

	if (strcmp(argv[0], "set_update_status") == 0)
		return set_update_status_main(argc, argv);
