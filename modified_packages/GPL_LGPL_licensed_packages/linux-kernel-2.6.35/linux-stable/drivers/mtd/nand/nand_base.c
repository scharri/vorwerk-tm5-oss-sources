...
static int nand_verify_erased_page(struct mtd_info *mtd, uint8_t *buf, int page)
{
	...
	struct gpmi_nfc_data    *this = (struct gpmi_nfc_data*)chip->priv;
	struct nfc_geometry     *geo = (struct nfc_geometry*)&this->nfc_geometry;
	
	int i, oobbits, databits;
	uint8_t *data = buf, *oob = chip->oob_poi;
	unsigned int max_bitflips = 0;
	
	unsigned int ecc_size = geo->ecc_chunk_size_in_bytes;
	unsigned int ecc_steps = geo->ecc_chunk_count;
	
	int oob_step = mtd->oobsize / ecc_steps;
	int ret;
	...
	if (ret) {
		printk(KERN_ERR "Error in chip->ecc.read_page_raw");
		return ret;
	}	

        unsigned int strength = geo->ecc_strength;
