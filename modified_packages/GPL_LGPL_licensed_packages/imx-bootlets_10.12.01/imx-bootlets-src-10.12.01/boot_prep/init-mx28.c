...
void change_gpmi_clock()
{
	int value = 0;

	/* Change ref_gpmi (PLL) to 480Mhz (18) */
	value = HW_CLKCTRL_FRAC1_RD();
	value &= ~BM_CLKCTRL_FRAC1_GPMIFRAC;
	value |= BF_CLKCTRL_FRAC1_GPMIFRAC (18);
	HW_CLKCTRL_FRAC1_WR (value);

	/* Change GPMI to 240Mhz (2) */
	value = HW_CLKCTRL_GPMI_RD();
	value &= ~BM_CLKCTRL_GPMI_DIV;
	value |= BF_CLKCTRL_GPMI_DIV (2);
	value &= ~BM_CLKCTRL_GPMI_DIV_FRAC_EN;		// 0 - enable integer divide
	HW_CLKCTRL_GPMI_WR (value);
	// wait till clock is really set
	while (HW_CLKCTRL_GPMI_RD() & BM_CLKCTRL_GPMI_BUSY);

	// Enable GPMI and parent clocks (order matters?)
	HW_CLKCTRL_FRAC1_CLR (BM_CLKCTRL_FRAC1_CLKGATEGPMI);
	HW_CLKCTRL_GPMI_CLR (BM_CLKCTRL_GPMI_CLKGATE);

	/* Select ref_gpmi (PLL) as clock source instead of XTAL */
...
void gpmi_init()
{
	...
	// wait till 1-> 0 (bring out of reset)
	HW_GPMI_CTRL0_CLR (BM_GPMI_CTRL0_SFTRST); while (HW_GPMI_CTRL0_RD() & BM_GPMI_CTRL0_SFTRST);
	// wait till 1-> 0 (ungate clkgate)
	HW_GPMI_CTRL0_CLR (BM_GPMI_CTRL0_CLKGATE); while (HW_GPMI_CTRL0_RD() & BM_GPMI_CTRL0_CLKGATE);
	
	// TODO: do we need to reset GPMI block here ?
	// reset and bring out of reset
	//HW_GPMI_CTRL0_SET (BM_GPMI_CTRL0_SFTRST); while (!(HW_GPMI_CTRL0_RD & BM_GPMI_CTRL0_SFTRST));
	//HW_GPMI_CTRL0_CLR (BM_GPMI_CTRL0_SFTRST); while (HW_GPMI_CTRL0_RD & BM_GPMI_CTRL0_SFTRST);
	// wait till 1-> 0 (ungate clkgate)
	//HW_GPMI_CTRL0_CLR (BM_GPMI_CTRL0_CLKGATE); while (HW_GPMI_CTRL0_RD & BM_GPMI_CTRL0_CLKGATE);
...
void gpmi_begin()
{
    ...
    unsigned int use_half_periods;
    unsigned int sample_delay_factor;
    unsigned int dll_value;
    unsigned int dll_wait_time_in_us;
    int value = 0;

	clock_frequency_in_hz = 240000000;
	clock_period_in_ns = 1000000000 / clock_frequency_in_hz;	// 4.16ns
	data_setup_in_cycles = 3; 	// 15ns
	data_hold_in_cycles = 3; 	// 12ns
	address_setup_in_cycles = 4; 	// 20ns
	sample_delay_factor = 12;
	use_half_periods = 0;
	dll_value = 1;
	...
	value = BF_GPMI_TIMING0_ADDRESS_SETUP(address_setup_in_cycles) | BF_GPMI_TIMING0_DATA_HOLD(data_hold_in_cycles) | BF_GPMI_TIMING0_DATA_SETUP(data_setup_in_cycles);
	HW_GPMI_TIMING0_WR (value);

	// 0x24a = 586, for period 4.16ns (240Mhz), (10ms for max. delay for program/erase / 4.16 / 4096)
	// TODO: but ROM loader only reads! This is probably not needed! Try without!
	//value = BF_GPMI_TIMING1_DEVICE_BUSY_TIMEOUT (587);
	value = BF_GPMI_TIMING1_DEVICE_BUSY_TIMEOUT (60);
	HW_GPMI_TIMING1_WR (value);


	/* Set registers used for high speed NAND access */
	...
	HW_GPMI_CTRL1_CLR (BM_GPMI_CTRL1_HALF_PERIOD);
	...
	if (sample_delay_factor)
	{
	    ...
	    if (use_half_periods)
		HW_GPMI_CTRL1_SET (BM_GPMI_CTRL1_HALF_PERIOD);

	    /* Set the delay factor. */
	    HW_GPMI_CTRL1_SET (BF_GPMI_CTRL1_RDN_DELAY(sample_delay_factor));
...
int _start(int arg)
{
...
#ifdef TM41_DEBUG
	// on TM41 board below lines enables debug output for boot_prep bootlet
	...
	*/
	...
	printf("start high speed GPMI\r\n");
	change_gpmi_clock();
	gpmi_init();
	gpmi_begin();
	printf("finish GPMI config\r\n");
