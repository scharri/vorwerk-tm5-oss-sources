...
{
        printk(KERN_INFO "mxs watchdog: mxs_wdt_disable()\n");
	spin_lock(&mxs_wdt_io_lock);
	__raw_writel(BV_RTC_PERSISTENT1_GENERAL__RTC_FORCE_UPDATER,
...
static int mxs_wdt_open(struct inode *inode, struct file *file)
{
	...
	printk(KERN_INFO "mxs watchdog: Opening watchdog\n");
	...
	if (len) {
		if (WATCHDOG_NOWAYOUT == 0) {
			...
			for (i = 0; i != len; i++) {
				...
				if (c == 'V') {
	                                printk(KERN_INFO "mxs watchdog: MAGIC CLOSE\n");
...
static int mxs_wdt_release(struct inode *inode, struct file *file)
{
	int ret = 0;
        ...
        printk(KERN_INFO "mxs watchdog: mxs_wdt_release()\n");
...
static int __devinit mxs_wdt_probe(struct platform_device *pdev)
{
	...
	if (ret < 0) {
		dev_err(&pdev->dev, "mxs watchdog: cannot register misc device\n");
		return ret;
	}

	printk(KERN_INFO "mxs watchdog: boot status %d\n",
		boot_status);
