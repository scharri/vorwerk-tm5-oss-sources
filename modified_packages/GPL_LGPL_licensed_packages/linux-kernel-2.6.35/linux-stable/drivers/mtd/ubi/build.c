...
//#ifdef CONFIG_MTD_UBI_DEBUG_PARANOID
	...
	// MICHAL - spawn separate bitflip-check task
	sprintf(ubi->bgt_bitflip_name, UBI_BGT_NAME_PATTERN, ubi->ubi_num + 1);
	ubi->bgt_bitflip_thread = kthread_create(ubi_bitflip_thread, ubi, ubi->bgt_bitflip_name);
	if (IS_ERR(ubi->bgt_bitflip_thread)) {
		err = PTR_ERR(ubi->bgt_bitflip_thread);
		ubi_err("cannot spawn \"%s\", error %d", ubi->bgt_bitflip_name,	err);
		goto out_uif;
	}
	// MICHAL - enable this thread from the very beginning
	wake_up_process(ubi->bgt_bitflip_thread);
...
int ubi_detach_mtd_dev(int ubi_num, int anyway)
{
	 ...
	 */
	if (ubi->bgt_bitflip_thread) {
		kthread_stop(ubi->bgt_bitflip_thread);
		ubi->bgt_bitflip_thread = NULL;
	}
	if (ubi->bgt_thread) {
