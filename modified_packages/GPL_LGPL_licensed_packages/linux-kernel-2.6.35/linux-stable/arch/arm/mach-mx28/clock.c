...
static int lcdif_set_rate(struct clk *clk, unsigned long rate)
{
	...
	for (div = 1; div < 256; ++div) {
		...
		if (fracdiv < 18 || fracdiv > 35) {
			ret = -EINVAL;
// pr_info("------------------------------------------------clocks ERROR clk->parent: %d\n",clk->parent->get_rate(clk->parent));
// pr_info("------------------------------------------------clocks ERROR fracdiv not between 18 and 35: %d\n",fracdiv);
			goto out;
		}
	...
	if (div >= 256 || lowest_result == (u32) -1) {
		ret = -EINVAL;
// pr_info("------------------------------------------------clocks ERROR div >=256: %d %d\n",div, lowest_result);
		goto out;
	}
	pr_info("Programming PFD=%u,DIV=%u ref_pix=%uMHz "
...
{
	int i;
	u32 reg_val;	// by michael 

// we select here PLL as clock source. Not 24MHz XTAL.
	...
	clk_enable(&cpu_clk);
	clk_enable(&emi_clk);
// MICHAL - increase GPMI speed 24M XTAL -> 240M PLL
	clk_set_rate(&ref_gpmi_clk, 480000000);
	clk_set_parent(&gpmi_clk, &ref_gpmi_clk);
	clk_set_rate(&gpmi_clk, 240000000);

	reg_val = __raw_readl(CLKCTRL_BASE_ADDR + HW_CLKCTRL_CLKSEQ);
	reg_val &= ~(BM_CLKCTRL_CLKSEQ_BYPASS_GPMI);
	__raw_writel(reg_val, CLKCTRL_BASE_ADDR + HW_CLKCTRL_CLKSEQ);
