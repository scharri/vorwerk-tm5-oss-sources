...
#warning "AUART2 enabled!!!!"
	 ...
	 },
	{
	 ...
	 {
	 .name  = "usb1_en",
	 .id    = PINID_SSP2_SS2, /* Power enable pin*/
	 .fun   = PIN_GPIO,
	 .strength	= PAD_4MA,
	 .voltage	= PAD_3_3V,
	 .data  = 0,
	 ...
	 .id    = PINID_SSP2_SS1, /* Overcurrent detection */
	 .fun   = PIN_FUN3,
	 },
#if 0
	{
...
#else
#if defined(CONFIG_INPUT_ALPMXS_ROTARY_ENCODER)
	{
	 .name          = "rotary-encoder-a1",
	 .id            = PINID_PWM0,
	 .fun           = PIN_GPIO,
	 .output        = 0,
	 .pullup        = 1,
...
#endif
#if defined(CONFIG_INPUT_MXS_ROTARY_ENCODER)
#warning "Compiling in HW rotary!"
	{
	 .name          = "rotary-encoder-a",
	 .id            = PINID_PWM0,
	 .fun           = PIN_FUN1,
	 .output        = 0,
	 .pullup        = 1,
...
static struct pin_desc mx28evk_eth_pins[] = {
	{
#warning "FEC enabled, this is not good!\n"
	 .name = "ENET0_MDC",
	 .id = PINID_ENET0_MDC,
	 .fun = PIN_FUN1,
	 .strength = PAD_8MA,
...
#  warning "Compiling in SPI collides with AlpsMXS Rotary!!!"
# endif
