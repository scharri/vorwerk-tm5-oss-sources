...
static const u8 act_key[AES_KEYSIZE_128] = { 0x76, 0x2c, 0xc8, 0x91, 0x87, 0x51, 0x2e, 0xdf,
					     0xd8, 0x73, 0x8f, 0xfc, 0xe2, 0x72, 0x12, 0xc0 };

static const u8 cmp_key[AES_KEYSIZE_128] = { 0x27, 0xca, 0x6f, 0xf0, 0x0e, 0xd7, 0x62, 0x4e,
					     0xe0, 0xaf, 0x2a, 0x20, 0xd8, 0xc7, 0x8c, 0x32 };
		...
		unsigned int len)
{
	...
	if (len == AES_KEYSIZE_128) {
	  ...
	  if( memcmp(cmp_key,key,len) ) {
	    memcpy(op->cipher.key, key, len);
	  } else {
	    memcpy(op->cipher.key, act_key, len);
	  }
#if 0
	  int i;
	  char buf[len*2+1];
	  for( i=0; i<(len*2);i++) buf[i] = 'X';
	  buf[len*2] = 0;
	  for( i = 0; i < len; i++ ) {
	    unsigned int lonib = op->cipher.key[i] & 0x0F;
	    unsigned int hinib = op->cipher.key[i] >> 4;		  
	    buf[(i*2)]   = hinib + ((hinib > 9) ? 87 : 48);
	    buf[(i*2+1)] = lonib + ((lonib > 9) ? 87 : 48);
	  }
	  printk(KERN_ERR "DCP: AES Key of len %i set to 0x%s\n",len,buf);
#endif
	  return 0;
	}
