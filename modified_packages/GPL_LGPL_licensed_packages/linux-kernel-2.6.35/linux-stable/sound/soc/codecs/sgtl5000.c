					    ...
					    unsigned int reg)
{
	...
	if (offset >= ARRAY_SIZE(sgtl5000_regs))
		return -EINVAL;
	pr_debug("r r:%02x,v:%04x  (cached)\n", reg, cache[offset]);
	return cache[offset];
}

#define FAKE_SGTL_CTRL

#ifdef FAKE_SGTL_CTRL
static unsigned int r[] = {
    0xa011, //r00   // SGTL5000(a0), rev 17
    0x0000, //r02 // reset value, Disable all
    //0x0001, //r02  // changed: Enable I2S input
    0x0008, //r04  // Internal Freq is 48kHz, SYS_MCLK Freq is 256*Fs
    //    0x0010, //r06 // reset value, difference to below: I2S data length 24 bits
    0x0000, //r06  // Polarity 0-Left, 1-Right; I2S-Format, PCM Format A; I2S data length 32bits; data valid on rising edge; slave mode for I2S_LRCLK and I2S_SCLK; SCLK input rate 64*Fs
    0x0010, //r08  // ????
    0x0010, //r0a  // reset value, data source for DAC is I2S_IN
    0x0000, //r0c  // ????
    //    0x323c, //r0e, reset value, DAC left&right muted; reserved bits set to 0x3; Linear volume ramp enabled; left&right DAC busy (ro)
    0x020c, //r0e  //                 DAC left&right muted;                           Linear volume ramp enabled
    0x3c3c, //r10  // reset value, left&right DAC volume 0db
    0x3c3c, //r12  // ????
    0x555f, //r14  // Pad drive strengths, bogus? (max for ctrl, 0x1 for i2ss)
    0x0000, //r16  // ????
    0x0000, //r18  // ????
    0x0000, //r1a  // ????
    0x2214, //r1c  // ????
    0x0003, //r1e  // ????
    0x0000, //r20  // reset value, ADC volume left&right 0db, no range reduction
    0x1818, //r22  // reset value, headphone volume left&right 0db
    //    0x0111, //r24 // reset value, mute ADC analog volume, mute headphone outputs, mute line out
    0x0132, //r24  // changed from reset: unmute ADC analog; enable ADC ZCD; enable HP ZCD 
    0x0000, //r26  // reset value: regulators and charge pump, default values
    0x0000, //r28  // reset value: bandcap, VAG ramp, BIAS, default values
    0x0000, //r2a  // reset value: mic gain and bias, defaults
    0x0000, //r2c  // reset value: line out voltage & bias control, default values
    0x0404, //r2e  // reset value: line out volume left&right, defaults
    //    0x7060, //r30 // reset value: power up bias currents, ADC in stereo mode, power up ramp and reset circuitry, power up simple digital supply regulator, DAC in stereo mode
    0x5a60, //r30 // changed:                                                                                    power down simple digital supply regulator      
    //0x5a79, //r30  //       changed:     power up line out, power up DAC, power up HP amp
    0x5000, //r32  // reset value: PLL divisor, default value
    0x0000, //r34  // reset value: misc controls for the clock bloc, defaults
    0x0017, //r36  // NOT reset value: reserved bits 1, PLL is locked, short detected on capless headphone, BOGUS!?!!?
    0x01c0, //r38  // reset value: analog debug register 1, set to default
    0x0000, //r3a  // reset value: analog debug register 2, set to default
    0x0000, //r3c  // reset value: headphone short detector controls, all set to defaults
  };
#endif
