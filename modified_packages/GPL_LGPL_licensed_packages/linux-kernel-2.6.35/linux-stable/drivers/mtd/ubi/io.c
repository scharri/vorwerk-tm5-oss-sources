			...
			int offset, int len)
{
	...
	len4 = len / 4;
	failed = 0;
	for (i = 0, i4 = 0; i4 < len4; i4++, i+=4) {
		uint32_t c = ((uint32_t *)buf)[i4];
		uint32_t c1 = ((uint32_t *)ubi->dbg_peb_buf)[i4];
	...
	if (failed) {
		int dump_len;
		printk(KERN_WARNING "MICHAL: ubi_dbg_check_write: verification failed !\n");
