...
struct mtd_data *mtd_open(const struct mtd_config *cfg, int flags)
{
	...
	for (i = 0; i < 2; i++) {
		...
		// MICHAL - is 0'ing necessary here? added just to be sure
		mp->nrbad = 0;
...
// MICHAL - disabled for now as probably not needed, as we use FCB blocks
		...
		int magic_offset)
{
	...
	for (ofs = *ofsp; ofs < end; ofs += stride) {
		...
		/* fast test */
		if (bbs->m_u32FingerPrint == magic1)
			break;
...
int mtd_load_all_boot_structures(struct mtd_data *md)
{
	...
	struct mtd_part *mp;
	int chip;
	unsigned int stride_size_in_bytes, search_area_size_in_bytes;
	unsigned int magic_offset = 12, magic_offset_dbbt = 0;
	...
	//----------------------------------------------------------------------
	// Check if the target MTD is too small to even contain the necessary search areas.
	//----------------------------------------------------------------------
	...
	chip = 0;
	ofs = 0;
	end = search_area_size_in_bytes;

	/* search up to 8 x 128kB blocks for FCB, first valid is taken */
	buf = mtd_load_boot_structure(md, chip, &ofs, end, FCB_FINGERPRINT, FCB_FINGERPRINT, FCB_FINGERPRINT, 0, magic_offset);
	if (buf == NULL) {
		fprintf(stderr, "mtd: FCB not found\n");
		return -1;
	} else {
		if (md->flags & F_VERBOSE)
			printf("mtd: valid FCB found @%d:0x%llx\n", chip, ofs);

		md->curr_fcb = (void *)(buf + magic_offset);
		md->fcb = *md->curr_fcb;
		md->curr_fcb = &md->fcb;
	}


	/* DBBTs are right after the FCBs */
	chip = 0;
	ofs = search_area_size_in_bytes;
	end = ofs + search_area_size_in_bytes;

	/* search up to 8 x 128kB blocks for DBBT, first valid is taken */
	buf = mtd_load_boot_structure(md, chip, &ofs, end, DBBT_FINGERPRINT2, DBBT_FINGERPRINT2, DBBT_FINGERPRINT2, 1, magic_offset_dbbt);
	if (buf == NULL) {
		fprintf(stderr, "mtd: DBBT not found\n");
		return -1;
	} else {
		if (md->flags & F_VERBOSE)
			printf("mtd: valid DBBT found @%d:0x%llx\n", chip, ofs);

		md->curr_dbbt28 = (void *)(buf + magic_offset_dbbt);
		md->dbbt28 = *md->curr_dbbt28;
		md->curr_dbbt28 = &md->dbbt28;
	}


	/* no bad blocks what-so-ever */
	if (md->curr_dbbt28->DBBT_Block.m_u32Number2KPagesBB == 0)
		return 0;

	/* BBTNs start here (4 pages offset in DBBT) */
	ofs = ofs + (4 * mtd_writesize(md));
	sz = mtd_writesize(md);
	bbtn = md->bbtn[0] = malloc(sz);
	if (md->bbtn[0] == NULL) {
		printf("mtd: UNABLE to allocate %d bytes for BBTN\n", sz);
		return -1;
	}

	/* read Bad Block Table from flash */

	r = mtd_read_page(md, chip, ofs, 1);
	if (r != sz) {
		fprintf(stderr, "mtd: read failed @0x%llx (%d)\n", ofs, r);
		return -1;
	}
	memcpy(bbtn, md->buf, sz);

	mp = &md->part[0];
	if ((bbtn->uNumberBB * mtd_erasesize(md)) > mp->info.size) {
		printf("mtd: illegal BBTN\n");
		return -1;
	}
...
#undef P1
#define P1(x)	printf("  %s = %d\n", #x, md->fcb.TM41_Block.x)
		printf("TM41\n");
		P1(m_u32UpdateStatus);
...
int v1_rom_mtd_init(struct mtd_data *md, FILE *fp, struct sw_version *sw_version_params)
{
	...
	// default to no update pending (0)
	fcb->TM41_Block.m_u32UpdateStatus            = 0;
	// after init both images will be the same (cloned)
	memcpy (&fcb->TM41_Block.images[0].version_date, sw_version_params->version_date, 20);
	memcpy (&fcb->TM41_Block.images[0].version_comment, sw_version_params->version_comment, 30);
	memcpy (&fcb->TM41_Block.images[1].version_date, sw_version_params->version_date, 20);
	memcpy (&fcb->TM41_Block.images[1].version_comment, sw_version_params->version_comment, 30);
	...
	dbbt28 = &(md->dbbt28);
	memset(dbbt28, 0, sizeof(*dbbt28));

	dbbt28->m_u32FingerPrint                = DBBT_FINGERPRINT2;
	dbbt28->m_u32Version                    = 0x01000000;

	/* Only check boot partition that ROM support (/dev/mtd0) */

	mp = &md->part[0];
	if (mp->nrbad == 0)
		return 0;

	md->bbtn[0] = malloc (mtd_writesize(md)); /* single page */
	if (md->bbtn[0] == NULL) {
		fprintf(stderr, "mtd: failed to allocate BBTN#%d\n", mtd_writesize(md));
		return -1;
	}
	...
	dbbt28->DBBT_Block.m_u32NumberBB = thisbad;  /* current number of bad blocks in this MTD partition */
	dbbt28->DBBT_Block.m_u32Number2KPagesBB = 1; /* one page should be enough */
		   ...
		   loff_t end, size_t size, int ecc)
{
	...
	// only one loop iteration - otherwise it will erase/write blocks double times
	for (i = 0; !err && i < 1; i++) {
...
{
	int startpage, start, size;
	unsigned int search_area_size_in_bytes, stride_size_in_bytes, search_area_size_in_strides;
	int i, r, chunk;
	loff_t ofs, end, bbtn_ofs;
	int chip = 0;
	...
	// if changing only committed image, ends here
	if (flags & UPDATE_FCB_ONLY)
	    return 0;
	...
	if (flags & UPDATE_DBBT) {
	    memset(md->buf, 0, mtd_writesize(md));
	    // calculate checksum for DBBT and copy content to buffer
	    bcb_calculate_checksum (&(md->dbbt28));
	...
	if (md->dbbt28.DBBT_Block.m_u32Number2KPagesBB > 0 && md->bbtn[0] != NULL) {
		...
		// the same number of times as DBBT (8), at 4 pages offset
		for (i = 0; i < search_area_size_in_strides; i++, ofs += stride_size_in_bytes) {
			bbtn_ofs = ofs + (4 * mtd_writesize(md));
			if (md->flags & F_VERBOSE)
				printf("mtd: PUTTING down DBBT%d BBTN%d @0x%llx (0x%x)\n", i, 0, bbtn_ofs, mtd_writesize(md));

			r = mtd_write_page(md, chip, bbtn_ofs, true);
			if (r != mtd_writesize(md)) {
				fprintf(stderr, "mtd: Failed to write BBTN @0x%llx (%d)\n", ofs, r);
			}
		}
...
void mtd_parse_args(struct mtd_config *cfg, int argc, char **argv)
{
	...
	for (i = 1; i < argc; i++) {
		...
		//break;
		fprintf(stderr, "mtd_parse_args(): ignoring argument '%s'\n", argv[i]);
	}
}
