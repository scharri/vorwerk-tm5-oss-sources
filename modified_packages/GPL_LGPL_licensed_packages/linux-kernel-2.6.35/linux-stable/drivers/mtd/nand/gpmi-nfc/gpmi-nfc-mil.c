...
static bool mil_erased_check(struct gpmi_nfc_data *this,
			unsigned char *data, unsigned int chunk, int page)
{
	...
	struct nand_chip          *chip     = &mil->nand;
	...
	if (flip_bits <= threshold) {
		dev_dbg(this->dev, "check for the erased page:%d, chunk:%d\n",
				page, chunk);
		flip_bits = 0;
		memset(buf, 0, geo->payload_size_in_bytes);

		/* read out the page without ECC enabled, and check it again. */
		chip->cmdfunc(mtd, NAND_CMD_READ0, 0, page);
		chip->read_buf(mtd, buf, geo->payload_size_in_bytes);

		for (i = 0; i < geo->payload_size_in_bytes; i++)
			flip_bits += hweight8(~buf[i]);

		if (flip_bits <= threshold)
			return true;
	} else
		if (flip_bits - threshold < 5)
			printk(KERN_WARNING "Found %u bit-flips in probably empty space", flip_bits);
				...
				uint8_t *buf, int page)
{
	...
	for (i = 0; i < nfc_geo->ecc_chunk_count; i++, status++) {
		...
		if (*status == 0xfe) {
			if (mil_erased_check(this, payload_virt, i, page)) {
				printk(KERN_WARNING "Correcting uncorrectable empty page.");
		...
		if (*status == 0xff) { /* Check erased page for bit-flips */
			flips = erased_sector_bitflips(payload_virt, i, nfc_geo);
			if (flips)
				printk(KERN_WARNING "Correcting %u bit-flips in erased page 0x%08X in chunk %u.", 
					flips, page, i);
		} 
		else
			flips = *status;
...
static int mil_scan_bbt(struct mtd_info *mtd)
{
	...
	pr_info("NAND Flash Device Information\n");
	pr_info("-----------------------------\n");
	nand_device_print_info(&this->device_info);
	#endif
	...
	pr_info("Page Size in Bytes     : %u\n", this->nfc_geometry.page_size_in_bytes);
	pr_info("Metadata Size in Bytes : %u\n", this->nfc_geometry.metadata_size_in_bytes);
	pr_info("ECC Chunk Size in Bytes: %u\n", this->nfc_geometry.ecc_chunk_size_in_bytes);
	pr_info("ECC Chunk Count        : %u\n", this->nfc_geometry.ecc_chunk_count);
	pr_info("Payload Size in Bytes  : %u\n", this->nfc_geometry.payload_size_in_bytes);
	pr_info("Auxiliary Size in Bytes: %u\n", this->nfc_geometry.auxiliary_size_in_bytes);
	pr_info("Auxiliary Status Offset: %u\n", this->nfc_geometry.auxiliary_status_offset);
	pr_info("Block Mark Byte Offset : %u\n", this->nfc_geometry.block_mark_byte_offset);
	pr_info("Block Mark Bit Offset  : %u\n", this->nfc_geometry.block_mark_bit_offset);
...
int gpmi_nfc_mil_init(struct gpmi_nfc_data *this)
{
	...
	nand->options |= NAND_ECC_NEED_CHECK_FF;
